...
export async function POST(request: Request) {
  try {
    ...
    const db = await getDb(); const recent = Date.now() - 15_000;
    ...
    // failure must not lock the user behind the resend cooldown.
    await db.update(otpChallenges).set({ consumedAt: Date.now() }).where(and(eq(otpChallenges.destination, destination), isNull(otpChallenges.consumedAt)));
